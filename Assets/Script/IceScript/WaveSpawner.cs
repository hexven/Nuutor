...
public class WaveSpawner : MonoBehaviour
{
    ...
    public CanvasGroup waveTextCanvasGroup; // NEW: CanvasGroup for fading wave text
    ...
    public float fadeDuration = 2f; // NEW: Duration of fade-out animation in seconds
    ...
    void Update()
    {
        ...
        if (spawnTimer <= 0 && enemiesToSpawn.Count > 0)
        {
            ...
            // Spawn an enemy with initial rotation
            GameObject enemy = Instantiate(enemiesToSpawn[0], spawnLocation.position, spawnRotation);
            // Track the spawned enemy
            activeEnemies.Add(enemy);
            Debug.Log($"Spawned enemy. Active enemies: {activeEnemies.Count}, To spawn: {enemiesToSpawn.Count}");
